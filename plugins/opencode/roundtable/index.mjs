...
const ROUNDTABLE_TOOL_IDS = new Set(['roundtable_native_roundtable']);
...
function appendRoundtableCommands(config) {
  ...
  const roundtableSpec = {
    ...
    template: [
      ...
      'Never call roundtable_native_roundtable in this command.',
    ].join('\n'),
  };
...
export const RoundtableOpenCodePlugin = async (ctx) => ({
  config: async (config) => {
    appendRoundtableCommands(config);
  },
  ...
  'tool.execute.after': async (input, output) => {
    ...
    if (!ROUNDTABLE_TOOL_IDS.has(String(input.tool))) return;
  ...
  tool: {
    roundtable_native_roundtable: tool({
      ...
      args: {
        ...
        persist_round_artifacts: tool.schema
          .boolean()
          .optional()
          .describe('Persist per-round evidence files under .roundtable/roundtable-artifacts (default true).'),
      ...
      async execute(args, toolContext) {
        ...
        if (strictCommand && !Boolean(args.allow_native)) {
          return JSON.stringify({
            success: false,
            error: `roundtable_native_roundtable is blocked during /${strictCommand.command}. Keep using task(...) + background_output(...) or call with allow_native=true.`,
          });
        }
        ...
        emitToolMetadata(toolContext, {
          title: `roundtable (${args.rounds ?? 2} rounds)`,
          metadata: {
            operation: 'roundtable-native',
        ...
        emitToolMetadata(toolContext, {
          title: `roundtable (${metadata.round_count} rounds, ${metadata.success_count}/${metadata.contribution_count} ok)`,
          metadata,
        });
