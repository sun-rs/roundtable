...
impl SessionStore {
    ...
    pub fn default_path() -> PathBuf {
        ...
        // - $XDG_DATA_HOME/roundtable/sessions.json
        // - ~/.local/share/roundtable/sessions.json
        let candidates = if let Some(base) = std::env::var_os("XDG_DATA_HOME") {
            let root = PathBuf::from(base);
            vec![root.join("roundtable").join("sessions.json")]
        } else if cfg!(windows) {
            if let Some(dir) = dirs::data_dir() {
                vec![dir.join("roundtable").join("sessions.json")]
            } else {
                Vec::new()
            }
        } else {
